...
pub fn table_reference(i: &[u8]) -> IResult<&[u8], Table> {
    map(pair(sql_identifier, opt(as_alias)), |tup| {
        // schema.table
        let schema_table = String::from_utf8_lossy(tup.0);
        let schema_table: Vec<&str> = schema_table.split('.').collect();

        Table {
            schema: if schema_table.len() > 1 { Some(String::from(schema_table[0])) } else { None },
            //name: String::from(str::from_utf8(tup.0).unwrap()),
            name: if schema_table.len() > 1 { String::from(schema_table[1]) } else { String::from(schema_table[0])  },
            alias: match tup.1 {
                Some(a) => Some(String::from(a)),
                None => None,
            },
...
mod tests {
    ...
    #[test]
    fn split_u8() {
        let arg = "test.tablename".as_bytes();
        //let result = arg.split(|s| s % '.' as u8 == 0);
        //dbg!(result);

        let schema = String::from_utf8_lossy(arg);
        let schema: Vec<_> = schema.split('.').collect();
        dbg!(schema[0]);
        dbg!(schema[1]);
    }
    ...
    fn take_while1_test() {
        ...
        //separated_pair()
        //let parser = separated_pair(tag("abc"), tag("."), tag("bcd"));
        //let result = parser("test.tablename");

        let result = alpha("test.tablename".as_bytes());

        dbg!(String::from_utf8_lossy(result.clone().unwrap().0));
        dbg!(String::from_utf8_lossy(result.clone().unwrap().1));
    }
    ...
    fn sql_identifiers() {
        ...
        let result = sql_identifier(id1);

        dbg!(String::from_utf8(result.unwrap().1.to_vec()));
